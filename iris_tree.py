...
class decisionnode:
    def __init__(self, dimension=None, threshold=None,results=None, NH=None,leftnode=None, rightnode=None, maxLabel=None):
        ...
        self.threshold = threshold   # 二分法的比较值
        self.results = results
        self.NH = NH  # 各节点样本量与经验值的乘积
        self.leftnode = leftnode  # 在d维，数据小于二分法比较值时，子树上的节点
        self.rightnode = rightnode  # 在d维，数据大于二分法比较值时，子树上的节点
        self.maxLabel = maxLabel  # 当前节点所含个数最多的属性
...
# 计算各属性中二分法所得几个阈值的最大增益
# data(原始数据) label(对应数据的属性)
# 返回各属性中具有最大信息增益的二分类值value，以及其最大信息增益gain
def labelmaxgain(data, labels, dimension):
...
# 根据计算不同属性在其最大二分类值下的信息增益，获取最大信息增益的属性选择类别
# 返回信息增益值，二分类数值以及对应属性
def maxgain(data, labels):
    Length = np.arange(len(data[0]))
    gainMax = 0
    valueMax = 0
    dimensionMax = 0
    for dimension in Length:
        gain, value = labelmaxgain(data, labels, dimension)
        if gainMax < gain:
            gainMax = gain
            valueMax = value
...
def buildDescisionTree(data, labels):
    if labels.size > 1:
        gain_max, value_max, dimension_max = maxgain(data, labels)
        if (gain_max > 0) :
